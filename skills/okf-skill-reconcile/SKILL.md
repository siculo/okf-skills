...
Follow `okf:_okf-skill-scan-bundle`. Use the returned index as the *bundle state*, including concept ID → frontmatter fields and body (body available in full mode; loaded on demand in two-pass mode).
...
## 12. Write invocation log

Follow `okf:_okf-skill-write-log` with:
- `skill`: `okf:reconcile`, `version`: `1.0`
- `bundle`: absolute path to the bundle root
- `outcome`: `success` if the final report was printed; `cancelled` if the user cancelled; `error` if an unrecoverable error occurred
- `concepts_added`: number of new concepts created
- `concepts_updated`: number of concepts updated
- `concepts_split`: number of concepts split
- `conflicts`: `null`
- `note`: `null`, or `"<N> stale concept(s) flagged"` if any stale concepts were found
